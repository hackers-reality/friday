...
def video_search(query: str, max_results: int = 10) -> str:
    """Find and open the best video match."""
    try:
        from duckduckgo_search import DDGS
        with DDGS() as ddgs:
            results = list(ddgs.videos(query, max_results=max_results))
        if not results:
            return "No visuals found."

        def score(v):
            s = 0
            t = (v.get("title", "") or "").lower()
            if any(k in t for k in ["official", "hd", "4k"]):
                s += 50
            if "youtube" in (v.get("publisher", "") or "").lower():
                s += 20
            return s

        best = max(results, key=score)
        open_url(best.get("content", ""))
        return f"Opening: {best.get('title', 'Video')}"
    except Exception as e:
        return f"Video Scout Error: {e}"

# ─── VISION TOOLS ─────────────────────────────────────────────────────────

def see_screen(question: str = "Analyze the current workspace.") -> str:
    """Visual Scout: Gemini Vision analyzes screen."""
    try:
    ...
    try:
        r = requests.post(
            ...
            json={
                "contents": [{
                    "parts": [
                        {"text": f"[STARK VISUAL SCOUT] {question}\nIdentify text, buttons, errors, coordinates for automation."},
