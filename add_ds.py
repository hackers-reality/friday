...
# Restore friday_tools.py from last good commit
import subprocess
subprocess.run(['git', 'checkout', '0af39bf', '--', 'friday_tools.py'], cwd='.')

# Read friday_tools.py
with open('friday_tools.py', 'r', encoding='utf-8') as f:
    tools_lines = f.readlines()

# Create properly indented deep_search_streaming function
ds_function = '''# ─── DEEP RESEARCH STREAMING ────────────────────────────────
...
# Append to friday_tools.py
with open('friday_tools.py', 'a', encoding='utf-8') as f:
    f.write('\n' + ds_function)

print('Added deep_search_streaming to friday_tools.py')
